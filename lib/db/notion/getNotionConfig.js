...
export async function getConfigMapFromConfigPage(allPages) {
  return getConfigMapFromPageType(
    allPages,
...
export async function getThemeConfigMapFromThemePage(allPages) {
  return getConfigMapFromPageType(
    allPages,
    ['THEME', 'theme', 'Theme'],
    '[Notion主题配置]'
...
export async function getConfigMapFromPageType(allPages, acceptedTypes, logLabel) {
  if (!allPages?.length) {
    console.warn(`${logLabel} 忽略的配置`)
    return null
  }

  const configPage = findConfigPageByType(allPages, acceptedTypes, logLabel)
  if (!configPage) return null

  const data = await fetchConfigPageData(configPage.id, logLabel)
  if (!data) return null
...
export function findConfigPageByType(allPages, acceptedTypes, logLabel) {
  const configPages = (allPages || []).filter(post =>
    post?.type && acceptedTypes.includes(post.type)
  )

  if (!configPages.length) {
    console.warn(`${logLabel} 未找到配置页面`)
    return null
  }
...
export async function fetchConfigPageData(configPageId, logLabel = '[Notion配置]') {
