...
import {
  ...
  getThemeConfigMapFromThemePage
...
async function convertNotionToSiteData(
  ...
  const NOTION_THEME_CONFIG = (await getThemeConfigMapFromThemePage(collectionData)) || {}
  if (process.env.NODE_ENV === 'development') {
    console.log(
...
function getCustomMenu({ collectionData, NOTION_CONFIG }) {
  ...
  if (menuPages && menuPages.length > 0) {
    menuPages.forEach(e => {
      console.log('[MenuOrder]', e.type, e.title, '| slug:', e.slug)
      e.show = true
      if (e.type === 'Menu') {
        menus.push(e)
