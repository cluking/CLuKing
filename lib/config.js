...
export const siteConfig = (key, defaultVal = null, extendConfig = {}) => {
  ...
  if (global) {
    ...
    if (hasVal(global.NOTION_CONFIG?.[key])) {
      val = global.NOTION_CONFIG[key]
    } else if (hasVal(global.NOTION_THEME_CONFIG?.[key])) {
      val = global.NOTION_THEME_CONFIG[key]
    } else if (hasVal(global.THEME_CONFIG?.[key])) {
